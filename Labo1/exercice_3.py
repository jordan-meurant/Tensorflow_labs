...
def exercice_3():
    x_data, target = load_csv_data("../Data/table_2_10.csv", names=['x1', 'x2', 'y'], target='y')
    ...
    # learn
    history = model.fit(x_data, target, epochs=50,
                        # callbacks=[EarlyStoppingAtMinLoss(0.725)],
                        verbose=1)
    ...
    plt.scatter(x='x1',
                y='x2',
                c=['r' if x else 'g' for x in (target < 0)],
                data=x_data
                )
